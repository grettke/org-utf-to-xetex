...
Org XeTeX Automate Font Specification By Unicode Block.
...
This package duplicates =ucharclasses=. It was exactly what I wanted but I couldn't get it working. It is also unmaintained. Being unmtainained isn't an issue in and of itself but it was /so easy/ to implement this functionality that I figured I would write something to do the same thing using Org-Mode and Elisp.
...
\DeclareTextFontCommand{\textEmoticons}{\Emoticons}
  #+END_SRC
- At this point XeTeX should render your characters using the correct font. Open that buffer and verify that your characters are wrapped with the correct font, it should look like the sample below. Verify this by exporting your document to a buffer calling /C-c C-e/ /l/ /L/. The should now render the characters correctly instead of using white boxes.
  #+BEGIN_SRC latex
\textEmoticons{😄} (Joy)
  #+END_SRC
